...
//				oMail.AddAttachment(sArchivo);
		...
		static public DataView Lista_VendedoresUnica(string personalName,C1.Data.C1DataSet ds,int iBodega)
		{
			return Funcion.dvProcedimiento(ds, string.Format("Exec Cre_clientesFilReiterativos '{0}',{1}", personalName,iBodega));	
		}
		...
		static public string AbreFacturacionClientes(int idTipoRuc, string sIdentificacion, bool bCancelar, bool bEditarNombre)
		{			
			string sRIdentificacion = "";
			
			using (frmFacturacionRegistroDeClientes FRC = new frmFacturacionRegistroDeClientes(idTipoRuc, sIdentificacion, bCancelar, bEditarNombre))
			{
 ...
 /**Incrementado por angélica */
		...
		{			
			string[] sParametros = new string[14];
			...
			if (dr.HasRows)
			{
				...
				sParametros[13] = dr.GetValue(13).ToString();
			}
			dr.Close();
		...
		static public string[] RetornaParametrosClientesProveedoresCrediDigital (bool bClienteProveedor, int idTipoFactura, string sIdentificacion, C1.Data.C1DataSet ds, int CrediDigital)
		{			

			string[] sParametros = new string[14];
			
			string sSQL = string.Format("Exec RetornaParametrosClientesProveedores {0}, {1}, '{2}', {3}", bClienteProveedor, idTipoFactura, sIdentificacion, CrediDigital);
			SqlDataReader dr = Funcion.rEscalarSQL(ds, sSQL, true);
			dr.Read();
			if (dr.HasRows)
			{
		...
		//Lista bodegas
		static public void ListaBodegas (Infragistics.Win.UltraWinGrid.UltraCombo uCombo, int idUsuario, int idTipoFactura, DateTime dtFecha, bool bRecibeConsignacion, string sconexion)
		{
			uCombo.DataSource = Funcion.dsEjecucion(sconexion, string.Format("Exec FacturacionListaBodegas {0}, {1}, '{2}', {3}", idUsuario, idTipoFactura, dtFecha.ToString("yyyyMMdd"), bRecibeConsignacion));
		}
		...
		{
			using (SqlConnection oConexion = new SqlConnection("Password=Bl45o6$9;Persist Security Info=True;User ID= 'infoelect';Initial Catalog=" + MenuLatinium.stDirFacturacion + ";Data Source=" + MenuLatinium.stDirServidor + ";Max Pool Size = 2048;"))
			{
				try
				{
		...
		static public void dsGeneralModulo(string sSQL, Infragistics.Win.UltraWinGrid.UltraGrid uGrid)
		{
			using (SqlConnection oConexion = new SqlConnection("Password=password.1;Persist Security Info=True;User ID= 'sa';Initial Catalog=" + MenuLatinium.stBDReplicacion + ";Data Source=" + MenuLatinium.stServidorReplicacion + ";Max Pool Size = 2048;"))
			{
				try
				{
					SqlCommand oCmdActualiza = oConexion.CreateCommand();
					...
					oCmdActualiza.Connection = oConexion;					
					oCmdActualiza.CommandTimeout = 1240;						
					oCmdActualiza.CommandText = sSQL;		
...
//			}
		...
		//Procedimientos para requerimiento
		static public DataTable dtEjecucionGeneral(string sconexion, string sSQL)
		{
