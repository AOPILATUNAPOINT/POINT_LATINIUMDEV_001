...
namespace Latinium
{
	...
	/// </summary>
	public class FreshLogistic : DevExpress.XtraEditors.XtraForm
	{
		...
		private void InitializeComponent()
		{
			...
			this.ultraGrid1.Size = new System.Drawing.Size(536, 288);
			...
			this.txtArchAsiento.Text = "C:\\FreshLogistic.xls";
			this.txtArchAsiento.ValueChanged += new System.EventHandler(this.txtArchAsiento_ValueChanged);
			// 
			// ultraButton2
			// 
			this.ultraButton2.Location = new System.Drawing.Point(280, 8);
			...
			// ultraButton3
			// 
			this.ultraButton3.Location = new System.Drawing.Point(8, 8);
			...
			// chkCreaCliente
			// 
			this.chkCreaCliente.Location = new System.Drawing.Point(176, 40);
			...
			// chkBorrarError
			// 
			this.chkBorrarError.Location = new System.Drawing.Point(56, 40);
			...
			this.Name = "FreshLogistic";
			this.Text = "Fresh Logistics";
		...
		{
			string stAudita = "Exec AuditaCrear 53, 6, 'FreshLog'";
	...
	//				CodVenta=CodVenta.Replace("#","."); 
				...
				else
				{
					...
					string detallito="Resumen de Facturas del " + dtFecha.ToString("dd-MMM-yyyy");
					...
					if(cuantos==0)
					{
						...
						stSelect = "Exec AsientoDetalleGenera "
							+ Efectivo.ToString("0.00", us) + ",0,-5," + IdAsiento.ToString() + ", @Detalle = 'Efectivo'"+ ", @Parametro = 5";
		...
		private void ultraButton3_Click(object sender, System.EventArgs e)
		{
			...
			OleDbDataAdapter myData = new OleDbDataAdapter("select Factura, NombreCliente AS Cliente, "
				+ " FechaFactura, FechaVenc, CodigoArticulo, Cantidad, Precio, Iva, NotaLinea, Credito, Borrar, Awb "
				+ "from [Hoja1$]", strConn);
			...
			{
				MessageBox.Show("No Debe tener abierto el archivo " + txtArchAsiento.Text + " en Excel",
					ex.Message);
				return;
			}
...
//			int IdCompraInicio1=0;
			...
			foreach(Infragistics.Win.UltraWinGrid.UltraGridRow dr in this.ultraGrid1.Rows.All)
			{
				...
				if (dr.Cells[3].Value != DBNull.Value)
					stFecha=dr.Cells[3].Value.ToString().Substring(0, 10);
				DateTime dtFechaVenc = Funcion.CreaFecha(stFecha);

				string sCantidad = dr.Cells[5].Value.ToString();
				int iCantidad=int.Parse (dr.Cells[5].Value.ToString());
				if( iCantidad==0) continue;
				
				double fTotal=float.Parse (dr.Cells[6].Value.ToString());
				string sPrecio = dr.Cells[6].Value.ToString();
				
				int iBorrar = int.Parse (dr.Cells[10].Value.ToString());
				int iContado = int.Parse (dr.Cells[9].Value.ToString());

				string stNotaLinea = dr.Cells[8].Value.ToString();
				string stOtro = dr.Cells[11].Value.ToString();
//				int iRecibe = int.Parse(dr.Cells[12].ToString());

				if(iFactura!=FactAnt) //es nueva factura			
				{
					stExec = "Exec CreaFactura '" + sCliente + "', '"
						+ "', '" + sFactura + "', '" + dtFecha.ToString("yyyyMMdd") + "',0,0,0,0,0"
						+ ", 1, " + iBorrar.ToString() + ", " + iContado.ToString() + ", 1, '', '"
						+ dtFechaVenc.ToString("yyyyMMdd") + "'";
				//	MessageBox.Show("compra:"+ stExec.ToString()  );  
					IdCompra = Funcion.iEscalarSQL(cdsCliente, stExec, true);

					if (IdCompra>0)
					{
						stExec = "Update Compra Set Otro = '" + stOtro + "'"
//							+ "', idRecibe = " + iRecibe.ToString()
				...
				if(IdCompra>0)
				{
					...
					double fIva=float.Parse (dr.Cells[7].Value.ToString());
					stExec = "Exec CreaFacturaDetalle " + sCantidad + ",'" + sCodigo + "', "
						+ fIva.ToString("0.00", us) + ", "
			...
			if(IdCompraInicio>0)
			{
				stExec = "EXEC FacturaTotales " + IdCompraInicio.ToString();
				Funcion.EjecutaSQL(cdsCliente, stExec, true);
			}
			stExec = "Select Count(*) From compra where idtipofactura=1";
			iCuenta = Funcion.iEscalarSQL(cdsCliente, stExec, true) - iCuenta;
			Cursor = Cursors.Default;
			MessageBox.Show("Proceso realizado exitosamente, se han creado " + iCuenta.ToString() + " Facturas ", " Factura");
		}
		...
		{
			string stAudita = "Exec AuditaCrear 53, 7, 'FreshLog'";
			Funcion.EjecutaSQL(cdsCliente, stAudita, true);
		}
