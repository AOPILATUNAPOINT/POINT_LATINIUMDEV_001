...
namespace Latinium
{
	...
	/// </summary>
	public class frmReporteDiarioCajaLotte : System.Windows.Forms.Form
	{
		...
		public frmReporteDiarioCajaLotte()
		{
			//
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmReporteDiarioCajaLotte));
			...
			this.lblUsuario.BackColor = System.Drawing.Color.DarkOrange;
			...
			this.lblContador.Location = new System.Drawing.Point(480, 43);
			...
			this.optTurno.Location = new System.Drawing.Point(384, 8);
			this.optTurno.Name = "optTurno";
			this.optTurno.Size = new System.Drawing.Size(208, 24);
			this.optTurno.TabIndex = 784;
			this.optTurno.Text = "Todos";
			this.optTurno.Visible = false;
			...
			this.label2.Font = new System.Drawing.Font("Tahoma", 10F, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((System.Byte)(0)));
			...
			this.label3.Font = new System.Drawing.Font("Tahoma", 10F, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((System.Byte)(0)));
			this.label3.ForeColor = System.Drawing.Color.Black;
			this.label3.Location = new System.Drawing.Point(728, 8);
			this.label3.Name = "label3";
			this.label3.Size = new System.Drawing.Size(120, 26);
			this.label3.TabIndex = 786;
			this.label3.Text = "DEUNA";
			this.label3.TextAlign = System.Drawing.ContentAlignment.MiddleCenter;
			// 
			// frmReporteDiarioCajaLotte
			// 
			...
			this.Name = "frmReporteDiarioCajaLotte";
		...
		private void frmReporteDiarioCaja_Load(object sender, System.EventArgs e)
		{
			...
			{				
				MessageBox.Show("No puede ingresar a Estado General De Caja", "Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Stop);
		...
		private void btnVer_Click(object sender, System.EventArgs e)
		{
			...
			if (MenuLatinium.stDirFacturacion == "GRETASANTORINI" || MenuLatinium.stDirFacturacion == "PROSERVICIOS")
			{
				string sSQL = string.Format("Exec ReporteDiarioCajaGreta '{0}', '{1}', {2}, {3}", 
					Convert.ToDateTime(this.dtDesde.Value).ToString("yyyyMMdd"), Convert.ToDateTime(this.dtHasta.Value).ToString("yyyyMMdd"), IBodega, (int) this.optTurno.Value);
		...
		private void DiseñoGridTurnoGreta()
		{
			try
			{				
				...
				this.uGridEstados.DisplayLayout.Bands[0].Columns["TurnoCaja"].Header.Caption = "Turno";
				this.uGridEstados.DisplayLayout.Bands[0].Columns["TurnoCaja"].Width = 60;
				this.uGridEstados.DisplayLayout.Bands[0].Columns["TurnoCaja"].CellActivation = Infragistics.Win.UltraWinGrid.Activation.Disabled;
				this.uGridEstados.DisplayLayout.Bands[0].Columns["TurnoCaja"].CellAppearance.TextHAlign = Infragistics.Win.HAlign.Center;
...
//        FuncionesProcedimientos.ListaBodegas(this.cmbBodega, MenuLatinium.IdUsuario, 45, (DateTime)this.dtHasta.Value, cdsSeteoF);
				this.cmbBodega.DataSource = Funcion.dvProcedimiento(cdsSeteoF, "Select Bodega, Nombre, Codigo From Bodega Where Activo = 1 And Caja = 1 AND (ISNULL (FechaCierre,'20301230') > '" + Convert.ToDateTime(this.dtHasta.Value).ToString("yyyyMMdd")+ "') Order By Nombre");
		...
		private void DiseñoGridTurno()
		{
			...
			{				
					...
					foreach(Infragistics.Win.UltraWinGrid.UltraGridRow dr in this.uGridEstados.Rows.All)
					{
						for (int i = 2; i < iContColumnas; i++)
						{
							...
							else if (dr.Cells[sColumna].Value.ToString().Trim().Length > 0) 
							{
								...
								{
									iContTarjetas ++;
									dr.Cells[sColumna].Appearance.BackColor = Color.DarkOrange; 
									dr.Cells[sColumna].Appearance.BackColor2 = Color.DarkOrange; 
								}	
								if(sNumero == "U")
								{
									iContDeUna ++ ;
									dr.Cells[sColumna].Appearance.BackColor = Color.SlateBlue;
									dr.Cells[sColumna].Appearance.BackColor2 = Color.SlateBlue;
								}
							}
						}
					}
				...
				this.lblContador.Text = iContAbierto.ToString()+ " Cajas Abiertas - " + iContCerrado.ToString() + " Cajas Cerradas -" + iContTarjetas.ToString() + " Cajas Tarjetas Conciliadas - " + iContDeUna.ToString() + " Cajas DeUna Conciliadas ";
			}
			catch(Exception Exc)
			{
				MessageBox.Show(string.Format("Error al Consultar: {0}", Exc), "Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Error);
		...
		{	
			if (MenuLatinium.stDirFacturacion == "GRETASANTORINI" || MenuLatinium.stDirFacturacion == "PROSERVICIOS"  )
			{
				...
				if (e.Cell.Column.ToString() == sColumna)
				{
					...
					if( (string) e.Cell.Row.Cells[sColumna].Value =="A" || (string)e.Cell.Row.Cells[sColumna].Value =="C" )
					{
						...
						using (frmResumenCierreCaja CCJ = new frmResumenCierreCaja((int)e.Cell.Row.Cells["bodega"].Value, dtFecha,(int)e.Cell.Row.Cells["TurnoCaja"].Value ))
						{
				...
				if ((string)e.Cell.Row.Cells[sColumna].Value == "U")
				{
					DateTime dtFecha = DateTime.Parse(sColumna + "/" +Convert.ToDateTime(this.dtHasta.Value).Month.ToString() + "/" + Convert.ToDateTime(this.dtHasta.Value).Year.ToString());

					using (frmDetalleDeUna frm1 = new frmDetalleDeUna(dtFecha,(int)e.Cell.Row.Cells["bodega"].Value,(string)e.Cell.Row.Cells["Local"].Value))
					{
				...
				else if ((string)e.Cell.Row.Cells[sColumna].Value == "T")
				{
					DateTime dtFecha = DateTime.Parse(sColumna + "/" + Convert.ToDateTime(this.dtHasta.Value).Month.ToString() + "/" + Convert.ToDateTime(this.dtHasta.Value).Year.ToString());

					using (frmDetalleTarjetasConciliadas frmTarjeta = new frmDetalleTarjetasConciliadas(dtFecha, (int)e.Cell.Row.Cells["bodega"].Value, (string)e.Cell.Row.Cells["Local"].Value))
					{
				...
				#endregion Otras empresas Greta
			}
			else
			{
				...
				if (e.Cell.Column.ToString() == sColumna)
				{
					...
					if( (string) e.Cell.Row.Cells[sColumna].Value =="A" || (string)e.Cell.Row.Cells[sColumna].Value =="T" )
					{
						DateTime dtFecha = DateTime.Parse (sColumna + "/" + Convert.ToDateTime(this.dtHasta.Value).Month.ToString() + "/" + Convert.ToDateTime(this.dtHasta.Value).Year.ToString());
					...
					if((string)e.Cell.Row.Cells[sColumna].Value == "C")
					{
						DateTime dtFecha = DateTime.Parse (sColumna + "/" + Convert.ToDateTime(this.dtHasta.Value).Month.ToString() + "/" + Convert.ToDateTime(this.dtHasta.Value).Year.ToString());
										
						using (frmDetalleTarjetasConciliadas CCJ = new frmDetalleTarjetasConciliadas( dtFecha, (int)e.Cell.Row.Cells["bodega"].Value, (string) e.Cell.Row.Cells["Local"].Value   ))
						{
							if (DialogResult.OK == CCJ.ShowDialog())
							{
