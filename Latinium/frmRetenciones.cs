...
namespace Latinium
{
	...
	public class frmRetenciones : DevExpress.XtraEditors.XtraForm
	{
		...
		private System.Windows.Forms.MenuItem mnuImpresion;
		private System.Windows.Forms.MenuItem mnuImpresionRIDE;
		CultureInfo us = new CultureInfo("en-US");

		public frmRetenciones(int idCompra, string factura, decimal dTotal, bool bCxP, DateTime dtFecha)
		{
			InitializeComponent();
		...
		public frmRetenciones(Infragistics.Win.UltraWinGrid.UltraGrid uGridCompra, int idCompra, int idCliente, string factura, DateTime dtFecha, decimal dIVA, decimal dTotal, bool bNuevo, int idComprobante, bool bLiquidacion)
		{
			//
		...
		private void InitializeComponent()
		{
			...
			this.mnuImpresion = new System.Windows.Forms.MenuItem();
			this.mnuImpresionRIDE = new System.Windows.Forms.MenuItem();
			...
			this.txtAutFactura.MaxLength = 50;
			...
			this.mnuImprimir.MenuItems.AddRange(new System.Windows.Forms.MenuItem[] {
																																								this.mnuImpresion,
																																								this.mnuImpresionRIDE});
			// 
			// mnuImpresion
			// 
			this.mnuImpresion.Index = 0;
			this.mnuImpresion.Text = "Retencion";
			this.mnuImpresion.Click += new System.EventHandler(this.mnuImpresion_Click);
			// 
			// mnuImpresionRIDE
			// 
			this.mnuImpresionRIDE.Enabled = false;
			this.mnuImpresionRIDE.Index = 1;
			this.mnuImpresionRIDE.Text = "RIDE";
			this.mnuImpresionRIDE.Click += new System.EventHandler(this.mnuImpresionRIDE_Click);
			// 
			// frmRetenciones
			// 
		...
		{
			iValRIDE = Funcion.iEscalarSQL(cdsSeteoF, string.Format("Select RIDERetenciones From SeteoF"));
			if (iValRIDE == 1) this.mnuImpresionRIDE.Enabled = true;
				...
				foreach(Infragistics.Win.UltraWinGrid.UltraGridRow dr in UGridCompra.Rows.All)
				{
					...
					if (IdComprobante == 1 || IdComprobante == 3)
					{
						...
						{								
							...
							string sSQLIVA = string.Format("Exec RetencionArticuloIVA {0}, {1}, '{2}', 4, {3}, {4}", idArticulo, dIVA, DTFecha.ToString("yyyyMMdd"), IdCliente, BLiquidacion);
					...
					#region Renta
							string sSQLRenta = string.Format("Exec RetencionArticuloRenta {0}, {1}, {2}, {3}", idArticulo, dRenta, IdCliente, BLiquidacion);
		...
		private void mnuImpresion_Click(object sender, System.EventArgs e)
		{
			Cursor = Cursors.WaitCursor;
		...
		private void mnuImpresionRIDE_Click(object sender, System.EventArgs e)
		{
//			if (Funcion.iEscalarSQL(cdsSeteoF, string.Format("Select r.idFE_EstadoComprobante from Retenciones r inner Join Compra c On r.idCompra = c.idCompra Where c.idCompra =  {0}", IdCompra)) != 2)
//			{
...
//			}
			...
			Reporte miRepor = new Reporte("RIDERetenciones.rpt", "");
			miRepor.MdiParent = this.MdiParent;
			miRepor.Titulo("Retenciones");
			miRepor.crVista.ParameterFieldInfo = paramFields;
			#endregion Impresion
