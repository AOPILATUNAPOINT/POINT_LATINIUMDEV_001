...
namespace Latinium
{
	...
	/// </summary>
	public class frmDetalleATS : DevExpress.XtraEditors.XtraForm
	{
		...
		private void InitializeComponent()
		{
			...
			this.dtPeriodo.CustomFormat = "dd/MM/yyyy";
			...
			this.Name = "frmDetalleATS";
			this.Text = "Anexo Transaccional Simplificado";
			this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.frmDetalleATS_KeyDown);
		...
		{			
			...
			if (!Funcion.Permiso("76", cdsSeteoF))
			{				
				MessageBox.Show("No puede ingresar a Anexos Transaccionales", "Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Stop);
				BeginInvoke(new MethodInvoker(UnloadMe));
				return;
			}

			this.dtPeriodo.Value = DateTime.Today; //MenuLatinium.dtFechaSistema;
			this.dtPeriodo.MaxDate = DateTime.Today; //MenuLatinium.dtFechaSistema;
				...
				{	
					...
					miXML.WriteElementString("numEstabRuc", Funcion.iEscalarSQL(cdsSeteoF, 
						string.Format("Select COUNT(*) From Bodega Where Activo = 1 And Factura = 1 And Principal = 1 And '{0}' Between Convert(Date, FechaApertura) And Convert(Date, ISNULL(FechaCierre, '20301231'))", 
							dtFecha.ToString("yyyyMMdd"))).ToString().PadLeft(3, Pad)); //+ Iva - DescPorcIva  - DescPorcIva0 
					...
					if (oDSCompras.Tables[0].Rows.Count >= 1)
					{
						...
						foreach(DataRow oDRCompras in oDSCompras.Tables[0].Rows)
						{
							...
							if (oDRCompras["TipoRuc"].ToString().Trim() == "03" && oDRCompras["Comprobante"].ToString().Trim() != "03")
							{
...
//							}
							...
							if ((decimal)oDRCompras["Total"] > 500.00m)
							{
...
//									MessageBox.Show(oDRVentas["Ruc"].ToString().Trim());
					...
					if (MenuLatinium.stDirFacturacion == "POINT")
					{
						...
						if (oDSVentasRB1.Tables[0].Rows.Count > 0)
						{
							...
							{				
								...
								if (oDRVentasRB1["CodComprobante"].ToString().Trim() == "18")
								{
									miXML.WriteStartElement("formasDePago");								
									miXML.WriteElementString("formaPago", "01");								
									miXML.WriteEndElement();
					...
					int iContBodegas = (int)Funcion.iEscalarSQL(cdsSeteoF, "Select Count(*) + 1 From Bodega");
		...
		private void btnCerrarImpuestos_Click(object sender, System.EventArgs e)
		{
			if ((int)this.txtEstado.Value == 0)
			{
				if (DialogResult.Yes == MessageBox.Show("El Cierre de Impuestos no puede ser revertido\n\n ¿Esta Seguro de Cerrar Impuestos?", "Point Technology", MessageBoxButtons.YesNo, MessageBoxIcon.Question)) 
				{
					...
					string sSQLIVA = string.Format("Exec GeneraAsientoImpuestosIVA '{0}', '{1}'", (int)this.txtidCierreImpuestos.Value, dtDesde.ToString("yyyyMMdd"), dtHasta.ToString("yyyyMMdd"));
					this.txtidAsientoIVA.Value = Funcion.iEscalarSQL(cdsSeteoF, sSQLIVA);

					string sSQLRenta = string.Format("Exec GeneraAsientoImpuestosRenta '{0}', '{1}'", (int)this.txtidCierreImpuestos.Value, dtDesde.ToString("yyyyMMdd"), dtHasta.ToString("yyyyMMdd"));
...
//			}
		...
		private void frmDetalleATS_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{
