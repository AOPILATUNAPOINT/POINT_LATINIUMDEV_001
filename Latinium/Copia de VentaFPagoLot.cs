...
namespace Latinium
{
	...
	public class VentaFPagoLot : DevExpress.XtraEditors.XtraForm
	{
		...
		public VentaFPagoLot(int idCompra, int idFormaPago, decimal  dValor, int iOrigen)
		{
			InitializeComponent();
		...
		public void VentaFPagoLot_Load(object sender, System.EventArgs e)
		{
			...
			bLoad = true;
			//this.cmbFormaPago.DataSource = Funcion.dvProcedimiento(cdsSeteoF, string.Format("Exec FormasDePagoIngresoDeCaja {0}, {1}, {2}, {3}, {4}, {5}", IdFormaDePago, 5, BAnticipoFact, BRetencionFact, BNotaDeCreditoFact, BCredito));
			this.cmbFormaPago.DataSource = Funcion.dvProcedimiento(cdsSeteoF, string.Format("Exec CompraFPagoCarga"));
			...
			if (!BPagoCredito)
			{
				...
				if (Funcion.iEscalarSQL(cdsSeteoF, string.Format("Select COUNT(*) From DetalleFormaPago Where idCompra = {0} And Origen = 1 and idFormaPago!=16", IdCompraFact)) == 0)
				{	
...
//			else e.Cell.Row.Cells["Valor"].Value = 0;
			...
			if (e.Cell.Column.ToString() == "idFormaPago")
			{
				...
				if (iFormaPago == 2 || iFormaPago == 3 || iFormaPago == 7 || iFormaPago == 11 || iFormaPago == 13 || iFormaPago == 16) // && e.Cell.Row.Cells["idFormaPago"].Value != DBNull.Value) //(decimal ) e.Cell.Row.Cells["Valor"].Value > 0 && 
				{
					#region Variables
					int IdBanco = 0;
					int IdTarjeta = 0;
					...
					using(ChequeTCLot miCheque = new ChequeTCLot(iFormaPago, IdBanco, IdTarjeta, IdCuenta, SNumero, DTFecha, SCuentaCorriente, SVoucher, SAutorizacion, SLote, SReferencia, IdPlan, IdPlazo))
					{
						if (DialogResult.OK == miCheque.ShowDialog())
						{
							...
							if (iFormaPago == 3 || iFormaPago == 11)
							{
								...
								e.Cell.Row.Cells["idPlan"].Value = 0; //(int)miCheque.cmbPlanes.Value;
							...
							{
								e.Cell.Row.Cells["DescripcionPrenda"].Value = (string)miCheque.txtDesPrenda.Value.ToString();
							}
							#endregion Prenda
						}
					}
...
//			}
		...
		private void ultraGrid1_BeforeRowsDeleted(object sender, Infragistics.Win.UltraWinGrid.BeforeRowsDeletedEventArgs e)
		{
			if (!bEliminaAlValidar)
			{
				for (int i = 0; i < e.Rows.Length; i++)
				{
					...
					if ((int)e.Rows[i].Cells["idFormaPago"].Value == 9)// || (int)e.Rows[i].Cells["idFormaPago"].Value == 10)
					{
						e.Cancel = true;
					}
		...
		private void ultraGrid1_ClickCellButton(object sender, Infragistics.Win.UltraWinGrid.CellEventArgs e)
		{
			...
			if (e.Cell.Column.ToString() == "I")
			{
				...
				if (iFormaPago == 2 || iFormaPago == 3 || iFormaPago == 7 || iFormaPago == 11 || iFormaPago == 13 || iFormaPago == 16)
				{
					#region Variables
					int IdBanco = 0;
					int IdTarjeta = 0;
			...
			if (e.Cell.Column.ToString() == "Valor")
			{
				if (e.Cell.Row.Cells["idFormaPago"].Value != System.DBNull.Value)
				{
					if ((int)e.Cell.Row.Cells["idFormaPago"].Value == 9 || (int)e.Cell.Row.Cells["idFormaPago"].Value == 12 
						|| (int)e.Cell.Row.Cells["idFormaPago"].Value == 10 || (int)e.Cell.Row.Cells["idFormaPago"].Value == 6)
					{
						e.Cancel = true;
					}
				}
