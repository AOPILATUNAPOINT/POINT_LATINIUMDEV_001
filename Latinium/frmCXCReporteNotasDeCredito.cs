...
namespace Latinium
{
	...
	public class frmCXCReporteNotasDeCredito : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.uGridDocumentos.Size = new System.Drawing.Size(1128, 336);
			...
			this.cmbMotivos.Location = new System.Drawing.Point(56, 72);
			...
			this.btnVer.Location = new System.Drawing.Point(344, 104);
			...
			this.cmbBodega.Appearance = appearance25;
		...
		private void btnVer_Click(object sender, System.EventArgs e)
		{
			...
			string sSQL = string.Format("Exec CXCReporteNotasDeCredito '{0}', '{1}', {2}, {3}, '{4}',{5}", 
				Convert.ToDateTime(this.dtDesde.Value).ToString("yyyyMMdd"), Convert.ToDateTime(this.dtHasta.Value).ToString("yyyyMMdd"), idMotivo, (int)this.rdgEstado.EditValue, this.txtBuscar.Text.ToString(), ibodega);
			...
			using (SqlConnection oConCobranzas = new SqlConnection("Password=Bl45o6$9;Persist Security Info=True;User ID= " + MenuLatinium.stUsuario + ";Initial Catalog=" + MenuLatinium.stDirFacturacion + ";Data Source=" + MenuLatinium.stDirServidor + ";Max Pool Size = 1024;"))
			{
				...
				{							
					...
					foreach(Infragistics.Win.UltraWinGrid.UltraGridRow dr in this.uGridDocumentos.Rows.All)
					{
						...
						if ((int)dr.Cells["idAsientoCCP"].Value > 0)
						{
							string sSQL = string.Format("Exec AsientoCruceAnticipoNCCrediPoint {0}, {1}, 1, {2}, 0, '{3}', {4}", 
								(int)dr.Cells["idCompra"].Value, (int)dr.Cells["idAsientoCCP"].Value, Convert.ToDecimal(dr.Cells["Total"].Value), Convert.ToDateTime(dr.Cells["FechaCruce"].Value).ToString("yyyyMMdd"), (int)dr.Cells["idOrigen"].Value);
