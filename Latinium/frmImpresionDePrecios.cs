...
namespace Latinium
{
	...
	public class frmImpresionDePrecios : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.cmbSubGrupo.Appearance = appearance19;
			...
			this.txtPosicion.Appearance = appearance21;
...
//				this.btnBuscar_Click(sender, e);
		...
		public static int idPreciadorConfiguracionImpresionPrecios = 1;
		private void btnImprimir_Click(object sender, System.EventArgs e)
		{
			bool bImpresionPorEcommerce = false;
			...
			try
			{
				string squery = String.Format("Exec PreciadorConfiguracionModel '{0}', {1}", 
				"Estado", 
				idPreciadorConfiguracionImpresionPrecios);
				bImpresionPorEcommerce = Funcion.bEjecutaSQL(cdsSeteoF, squery);

				squery = String.Format("Exec PreciadorConfiguracionModel '{0}', {1}", 
					"rutaServicio", 
					idPreciadorConfiguracionImpresionPrecios);
				rutaServicio = Funcion.sEscalarSQL(cdsSeteoF, squery);

				impresoraPredeterminada = MenuLatinium.ImpresoraNombre;
			...
			{		
				try
				{
					...
					string squeryrutaSegura = String.Format("Exec PreciadorConfiguracionModel '{0}', {1}", 
						"RutaDestinoPdf", 
						idPreciadorConfiguracionImpresionPrecios);
					string rutaSegura = Funcion.sEscalarSQL(cdsSeteoF, squeryrutaSegura);
					Directory.CreateDirectory(rutaSegura); // Crea la carpeta si no existe

					// Generar la URL del servicio
					string urlServicio = string.Format("{2}documentos/generar-pdf/{0}/{1}",
						(int)this.cmbBodega.Value,
						(int)this.rdgPreciador.EditValue,
						rutaServicio
						);

					try
        {
            // Crear una solicitud HTTP al servicio
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(urlServicio);
            request.Method = "GET"; // Puedes usar POST si el servicio lo requiere
            ...
            using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
            {
                ...
                {
                    Console.WriteLine("El servicio se ejecutó correctamente.");
                }
                else
                {
                    Console.WriteLine("Error en el servicio:" + response.StatusCode);
                }
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    Console.WriteLine("Respuesta del servicio: " + responseBody);
										string tmpDocumento = responseBody;
										// Esperar un tiempo para que se genere el archivo
										//Thread.Sleep(10000);
										if (tmpDocumento != "")
										{
											Process.Start("cmd.exe", "/C" + "net use \\192.168.1.10  /user:latinium 10dgpv15.p");
											// Mover el archivo generado a la ruta segura
											string archivoDestino = Path.Combine(rutaSegura, Path.GetFileName(tmpDocumento));
											File.Copy(tmpDocumento, archivoDestino, true);
											// Eliminar el archivo PDF después de imprimir
											try
											{
												if (File.Exists(tmpDocumento))
												{
													File.Delete(tmpDocumento);
											...
											string nombreImpresora = MenuLatinium.ImpresoraNombre;
											ImprimirPDF(archivoDestino, nombreImpresora);
											MessageBox.Show("El proceso de impresión ha sido iniciado.", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
										}
										else
										{
											MessageBox.Show("No se encontró el documento generado.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
										}
										MessageBox.Show("Imprimiendo precios\n\npor favor espere", "Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Information);
										// Eliminar el registro temporal en la base de datos
										...
										// Actualizar el grid
										string sSQLAG = string.Format("Exec BuscaListaDePrecios 0, {0}, {1}, ''",
											(int)this.cmbBodega.Value, (int)this.rdgPreciador.EditValue);
        ...
        {
            Console.WriteLine("Error al ejecutar el servicio: " + ex.Message);
        }
		...
		private void ImprimirPDF(string rutaPDF, string nombreImpresora)
		{
			try
			{
				if (!File.Exists(rutaPDF))
				{
					System.Windows.Forms.MessageBox.Show("El archivo PDF no existe.", "Error", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Error);
					return;
				}
				
				string squeryrutaRed = String.Format("Exec PreciadorConfiguracionModel '{0}', {1}", 
					"RutaInstaladorSumatraPDF", 
					idPreciadorConfiguracionImpresionPrecios);
				string rutaRed = Funcion.sEscalarSQL(cdsSeteoF, squeryrutaRed);

				string squeryrutaDestino = String.Format("Exec PreciadorConfiguracionModel '{0}', {1}", 
					"RutaDestinoSumatraPDF", 
					idPreciadorConfiguracionImpresionPrecios);
				string rutaDestino = Funcion.sEscalarSQL(cdsSeteoF, squeryrutaDestino);
        ...
        if (!File.Exists(rutaDestino))
        {
            try
            {
                // Si no existe, copiarlo desde la red
                File.Copy(rutaRed, rutaDestino);
        ...
        // Ejecutar el archivo SumatraPDF desde la ruta de destino
        try
        {
					Process proceso = new Process();
					proceso.StartInfo.FileName = rutaDestino;
					proceso.StartInfo.Arguments = string.Format("-print-to \"{0}\" \"{1}\"", nombreImpresora, rutaPDF);
					proceso.StartInfo.UseShellExecute = false;  // No usar el shell para evitar que aparezca la ventana
					proceso.StartInfo.CreateNoWindow = true;  // No mostrar ninguna ventana de SumatraPDF
					proceso.Start();
					// Espera a que termine el proceso (la impresión se enviará antes de que el proceso termine)
					proceso.WaitForExit();
					Console.WriteLine("SumatraPDF ejecutado y PDF enviado a la impresora.");
        }
        catch (Exception ex)
        {
           Console.WriteLine("Error al ejecutar el programa: " + ex.Message);
        }
				// Eliminar el archivo PDF después de imprimir
				try
				{
				...
				System.Windows.Forms.MessageBox.Show("El archivo PDF se ha enviado a imprimir correctamente.", "Éxito", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information);
			}
			catch (Exception ex)
			{
				System.Windows.Forms.MessageBox.Show("Error al imprimir el archivo PDF: " + ex.Message, "Error", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Error);
			}
		}
