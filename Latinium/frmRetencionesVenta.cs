...
namespace Latinium
{
	...
	public class frmRetencionesVenta : System.Windows.Forms.Form
	{
		...
		public frmRetencionesVenta(int idRetencion, int idCompra, string factura, decimal dTotal, DateTime dtFecha, int iCont)
		{
			//
			// Necesario para admitir el Diseñador de Windows Forms
			//
		...
		private void frmRetencionesVenta_Load(object sender, System.EventArgs e)
		{
			...
			if (IdRetencion == 0)
			{
				...
				{					
					if (IdCliente == 0) 
					{
						IdCliente = Funcion.iEscalarSQL(cdsSeteoF, string.Format("select idCliente from compra where idCompra = {0}", IdCompra));
					}
