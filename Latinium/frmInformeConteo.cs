...
namespace Latinium
{
	...
	public class frmInformeConteo : DevExpress.XtraEditors.XtraForm
	{
		...
		private void InitializeComponent()
		{
			...
			this.uGridInforme.DisplayLayout.Override.CellAppearance = appearance7;
			...
			this.uGridInforme.DisplayLayout.Override.HeaderAppearance = appearance8;
			...
			this.uGridInforme.DisplayLayout.Override.RowSelectorAppearance = appearance9;
			appearance10.BackColor = System.Drawing.Color.FromArgb(((System.Byte)(251)), ((System.Byte)(230)), ((System.Byte)(148)));
			appearance10.BackColor2 = System.Drawing.Color.FromArgb(((System.Byte)(238)), ((System.Byte)(149)), ((System.Byte)(21)));
			appearance10.BackGradientStyle = Infragistics.Win.GradientStyle.Vertical;
			this.uGridInforme.DisplayLayout.Override.SelectedRowAppearance = appearance10;
			...
			this.chkPaso1.Appearance = appearance11;
			...
			this.chkPaso2.Appearance = appearance12;
			...
			this.chkPaso4.Appearance = appearance13;
			...
			this.chkPaso3.Appearance = appearance14;
			...
			this.chkPaso5.Appearance = appearance15;
			...
			appearance16.BackColorDisabled2 = System.Drawing.Color.Transparent;
		...
		private void btnGuardar_Click(object sender, System.EventArgs e)
		{
			...
			foreach (UltraGridRow row in uGridInforme.Rows)
			{
				if (row.Cells["idEstado"].Value != null && Convert.ToInt32(row.Cells["idEstado"].Value) == 3)
				{
					int idArticulo = Convert.ToInt32(row.Cells["idArticulo"].Value);

					string sSQL = string.Format("EXEC ObtenerPrecio3Articulo {0}", idArticulo);
					decimal precio3 = Funcion.decEscalarSQL(cdsSeteoF, sSQL);

					decimal diferencia = 0;
					if (row.Cells["ConteoExistencia"].Value != null)
						diferencia = Math.Abs(Convert.ToDecimal(row.Cells["ConteoExistencia"].Value));
					totalFaltantes += (precio3 * diferencia);
				}
			}
			decimal iva = Math.Round(totalFaltantes * 0.15m, 2);
			totalFaltantes += iva;
			if (totalFaltantes == 0)
			{
				if (Convert.ToInt32(this.txtEstado.Value) == 2)
				{
					MessageBox.Show("Conteo de inventario cerrado sin faltantes.", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
					return;
				}
				else
				{
					DialogResult result = MessageBox.Show(
						"No hay diferencias para facturar. El conteo de inventario se cerrará. ¿Desea continuar?","Confirmación",
			...
			using (frmFacturacionFaltantes Personal = new frmFacturacionFaltantes((int)this.txtidCierre.Value, dtFecha, Bodega , totalFaltantes,(int)this.txtEstado.Value))
			{
				...
				else if ((int)this.uGridInforme.ActiveRow.Cells["idEstado"].Value == 4)
				{
					...
					else
					{
						using (frmCruceConteoInventario Cruce = new frmCruceConteoInventario((int)this.uGridInforme.ActiveRow.Cells["idArticulo"].Value, (int)this.uGridInforme.ActiveRow.Cells["idArticulo"].Value, Bodega, dtFecha, this.uGridInforme.ActiveRow.Cells["Codigo"].Value.ToString(), (int)this.uGridInforme.ActiveRow.Cells["ConteoExistencia"].Value, (int)this.uGridInforme.ActiveRow.Cells["CantidadJustifica"].Value))
						{
							...
							{
								string sSQL = string.Format("Exec	ConsultaDiferenciasInventario '{0}', {1}, {2}", 
									dtFecha.ToString("yyyyMMdd"), Bodega, (int)this.txtidCierre.Value);
								this.uGridInforme.DataSource = Funcion.dvProcedimiento(cdsSeteoF, sSQL);	
							}
						}
					}
		...
		private void ImprimirSerie ( )
		{
			...
			Reporte miRepor = new Reporte("ReporteSerieConteo.rpt", "");
			miRepor.MdiParent = this.MdiParent;			
			miRepor.crVista.ParameterFieldInfo = paramFields;
			miRepor.ShowDialog();
		}
		...
		{		
			//int Local = Funcion.iEscalarSQL(cdsSeteoF, string.Format("Exec [ConsultaAuditoriaLocalesBodega] {0}",  Bodega)); 

			using (frmReporteAuditoriaLocal1 AuditoriaL = new frmReporteAuditoriaLocal1(Bodega, dtFecha, (int)this.txtEstado.Value, 0))
			{
