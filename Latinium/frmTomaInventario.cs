...
namespace Latinium
{
	...
	public class frmTomaInventario : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.grdDTomas.InitializeLayout += new Infragistics.Win.UltraWinGrid.InitializeLayoutEventHandler(this.grdDTomas_InitializeLayout);
			// 
			// ultraDataSource2
			// 
			...
			this.btnVer.Location = new System.Drawing.Point(504, 40);
			this.btnVer.Name = "btnVer";
			this.btnVer.Size = new System.Drawing.Size(56, 23);
			this.btnVer.TabIndex = 614;
			this.btnVer.Text = "Serie";
			this.btnVer.TextAlign = System.Drawing.ContentAlignment.MiddleLeft;
			this.toolTip1.SetToolTip(this.btnVer, "Buscar información...!!!");
			this.btnVer.Click += new System.EventHandler(this.btnVer_Click);
			this.btnVer.ContextMenuChanged += new System.EventHandler(this.btnVer_ContextMenuChanged);
			// 
			// label4
			// 
		...
		private void frmTomaInventario_Load(object sender, System.EventArgs e)
		{
			...
			drBodega.Close();
			 ...
			 iEstado = Funcion.iEscalarSQL(cdsSeteoF, 
				string.Format("Select Estado From CierreConteoInventario Where Bodega = '{0}' And CONVERT(DATE, Fecha) = '{1}'", 
		...
		private void btnRegistrar_Click(object sender, System.EventArgs e)
		{
			...
			if ((int)this.txtIdArticulo.Value > 0) 
			{
				...
				if (bManejaSerial)
				{
					...
					string sSQLVSEB = string.Format("Exec ValidaSerialEnBodegaInventario '{0}', {1}, {2}", this.txtSerial.Text.ToString(), iBodega, (int)this.txtIdArticulo.Value);
			...
			if (bManejaSerial)
			{
				#region Valida Serie en Compra

				int iValidaSerie = 0;
				iValidaSerie = Funcion.iEscalarSQL(cdsSeteoF, string.Format("Exec [ValidaSerieODC] '{0}'", this.txtSerial.Text.ToString()));	
				if ( iValidaSerie <= 0)
				{
					MessageBox.Show("La Serie no fue Ingresada desde una Factura de Compra", "Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Stop);
					this.txtSerial.Focus();
					return;
				}
				#endregion Valida Serie en Compra
			}
		...
		private void btnVerToma_Click(object sender, System.EventArgs e)
		{
			...
			if( iEstado == 1 ) 
			{
				string sSQL = string.Format("Exec [Visualiza_TomaInventario_Abierto] {0}, {1}, '{2}', '{3}'", 
					1, iBodega, Convert.ToDateTime(this.txtFecha.Value).ToString("yyyyMMdd"), this.txtCodigo.Text);

				FuncionesProcedimientos.dsGeneralModulo(sSQL, this.grdDTomas);
				return;
			...
			if( iEstado == 2 ) 
			{
				string sSQL = string.Format("Exec [Visualiza_TomaInventario] {0}, {1}, '{2}', '{3}'", 
					1, iBodega, Convert.ToDateTime(this.txtFecha.Value).ToString("yyyyMMdd"), "");
		...
		private void grdDTomas_InitializeLayout(object sender, Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs e)
		{
		
		}

		private void btnVer_ContextMenuChanged(object sender, System.EventArgs e)
		{
		...
		private void btnVer_Click(object sender, System.EventArgs e)
		{
			...
			if ((int) this.txtIdArticulo.Value > 0)
			{
				...
				if (bManejaSerial)
				{
					using (frmSerieTomaInventario Serie = new frmSerieTomaInventario((int) this.txtIdCierre.Value ,iBodega, (int) this.txtIdArticulo.Value, this.txtCodigo.Text, Convert.ToInt32( this.optBodegaExhibicion.Value), (DateTime)this.txtFecha.Value, (int)this.txtIdCierre.Value ))
					{
						if (DialogResult.OK == Serie.ShowDialog())
						{
