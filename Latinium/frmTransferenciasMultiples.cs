...
namespace Latinium
{
	...
	public class frmTransferenciasMultiples : DevExpress.XtraEditors.XtraForm
	{
		...
		private void InitializeComponent()
		{
			...
			this.uGridTransferencias.Location = new System.Drawing.Point(240, 120);
			this.uGridTransferencias.Name = "uGridTransferencias";
			this.uGridTransferencias.Size = new System.Drawing.Size(920, 184);
			...
			// btnBuscar
			// 
			this.btnBuscar.Location = new System.Drawing.Point(376, 72);
			...
			this.label61.Location = new System.Drawing.Point(200, 40);
			...
			this.txtRangoDias.Location = new System.Drawing.Point(288, 40);
			...
			this.uGridVentas.Size = new System.Drawing.Size(904, 267);
			...
			this.txtDiasAbastecimiento.Location = new System.Drawing.Point(504, 40);
			...
			this.uGridResumen.Size = new System.Drawing.Size(920, 315);
			...
			this.lblEstado.Location = new System.Drawing.Point(480, 72);
			...
			this.uGridLocales.Location = new System.Drawing.Point(8, 120);
			this.uGridLocales.Name = "uGridLocales";
			this.uGridLocales.Size = new System.Drawing.Size(224, 507);
			...
			this.chkTodos.Location = new System.Drawing.Point(200, 144);
		...
		private void frmTransferenciasMultiples_Load(object sender, System.EventArgs e)
		{
			...
			this.cmbBodega.DataSource = Funcion.dvProcedimiento(cdsSeteoF, "Select b.Bodega, b.Nombre FROM Bodega b WHERE ORIGENTM = 1");
		...
		private void btnCalcular_Click(object sender, System.EventArgs e)
		{
			...
			if (DialogResult.Yes == MessageBox.Show("¿Desea Crear un nuevo abastecimiento?", "Point Technology", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2)) bNuevo = true;

			string sSQL = string.Format("Exec CalculoTransferenciasAutomaticas '{0}', {1}, {2},'{3}', {4}, {5}", dtFecha.ToString("yyyyMMdd"), (int)this.txtRangoDias.Value, (int)this.txtDiasAbastecimiento.Value, sLocales, (int)this.cmbBodega.Value, bNuevo);	
...
//							this.uGridTransferencias.DisplayLayout.Bands[0].Columns[nombrCol].Header.Caption = nombrCol.Substring(0, 2);
		...
		{
			FuncionesProcedimientos.ExportaExcel(this.uGridTransferencias);
		}	
		...
		private void btnBuscar_Click(object sender, System.EventArgs e)
		{
			...
			string sSQL = string.Format("Exec TFMBuscaCodigo '{0}', {1}", 
				this.txtBuscar.Text.ToString(), (int)this.cmbBodega.Value);
			FuncionesProcedimientos.dsGeneralModulo(sSQL, this.uGridTransferencias);
		...
		private void btTransferir_Click(object sender, System.EventArgs e)
		{
			...
			sBodega = Funcion.sEscalarSQL(cdsSeteoF, string.Format("Select Codigo from Bodega where Bodega = {0}", (int) this.cmbBodega.Value));		
			...
			foreach(Infragistics.Win.UltraWinGrid.UltraGridRow dr in this.uGridTransferencias.Rows.All)
			{
				if ((bool)dr.Cells["Sel"].Value)
				{
					int iSaldo = Funcion.iEscalarSQL(cdsSeteoF, string.Format("Select dbo.RetornaExistencia('{0}', {1}, {2}, {3}, 8, 0)", 
						DateTime.Today.ToString("yyyyMMdd"), 0, (int) this.cmbBodega.Value, (int)dr.Cells["idArticulo"].Value));
			...
			if (DialogResult.Yes == MessageBox.Show("¿Esta seguro de generar las transferencias?", "Point Technology", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2))
			{
				...
				string sSQL = string.Format("Exec TFMBuscaCodigo '', {0}", (int)this.cmbBodega.Value);
				FuncionesProcedimientos.dsGeneralModulo(sSQL, this.uGridTransferencias);
								...
								if (totColumna > 0)
								{
									...
									foreach(Infragistics.Win.UltraWinGrid.UltraGridRow dr in this.uGridTransferencias.Rows.All)
									{
										if ((int)dr.Cells[coltran].Value > 0 && (bool)dr.Cells["Sel"].Value)
										{
											...
											if (swpaso == 1)
											{									
												string stExec = string.Format("Exec TransformaIE_TM_Cabecera '{1}', '{0}'", coltran, sBodega);
...
//												if (iContFila == 26) /* quitar el lunes  */
//												{
...
//				else e.Cell.Row.Cells["Sel"].Value = true;
		...
		private void btnActualizaSaldo_Click(object sender, System.EventArgs e)
		{
			if (DialogResult.Yes == MessageBox.Show("Esta Seguro de Actualizar Existencias en la Bodega Principal", "Point Technology", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2))
			{
				...
				FuncionesProcedimientos.dsGeneralModulo(string.Format("Exec TMActualizaExistenciaBI {0}", (int)this.cmbBodega.Value), this.uGridTransferencias);
		...
		private void cmbBodega_ValueChanged(object sender, System.EventArgs e)
		{
			if (this.cmbBodega.ActiveRow != null)
			{
				...
				{
					this.uGridLocales.DataSource = Funcion.dvProcedimiento(cdsSeteoF, "Select b.Bodega, b.Nombre , CONVERT ( Bit, 1) as Sel FROM Bodega b WHERE inventario = 1 and TransferenciaMultiple = 1 and Sel = 0 and Region in (1,2,3) and OrigenTm <> 1 order by b.Nombre ");
				}
				else
				{
					this.uGridLocales.DataSource = Funcion.dvProcedimiento(cdsSeteoF, "Select b.Bodega, b.Nombre , CONVERT ( Bit, 1) as Sel FROM Bodega b WHERE inventario = 1 and TransferenciaMultiple = 1 and Sel = 0 and Region in (2) and OrigenTm <> 1 order by b.Nombre  ");
				}
