...
namespace Latinium
{
	...
	public class UsuarioClave : DevExpress.XtraEditors.XtraForm
	{
		...
		private void btAceptar_Click(object sender, System.EventArgs e)
		{
			...
			#region Valida estructura clave
			string claveActual = this.txtClaveNueva.Text.Trim();
			...
			if (claveActual.Length < 8 || claveActual.Length > 12)
			{
				MessageBox.Show("La Clave Actual debe tener entre 8 y 12 caracteres", "Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Stop);
				this.txtClaveNueva.Focus();				
				return;
			}

			// Validar que contiene al menos un número
			bool contieneNumero = false;
			for (int i = 0; i < claveActual.Length; i++)
			{
				if (char.IsDigit(claveActual[i]))
				{
					contieneNumero = true;
					break;
				}
			...
			if (!contieneNumero)
			{
				MessageBox.Show("La Clave Actual debe contener al menos un número", "Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Stop);
				this.txtClaveNueva.Focus();				
				return;
			}
			...
			for (int i = 0; i < claveActual.Length; i++)
			{
				if (char.IsLetter(claveActual[i]))
				{
					contieneLetra = true;
					break;
				}
			...
			if (!contieneLetra)
			{
				MessageBox.Show("La Clave Actual debe contener al menos una letra (A-Z)", "Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Stop);
				this.txtClaveNueva.Focus();				
				return;
			}

			// Validar que contiene al menos un carácter especial
			bool contieneEspecial = false;
			string especiales = ".@#$!";

			for (int i = 0; i < claveActual.Length; i++)
			{
				if (especiales.IndexOf(claveActual[i]) >= 0)
				{
			...
			if (!contieneEspecial)
			{
				MessageBox.Show("La Clave Actual debe contener al menos un carácter especial permitido. Caracteres permitidos: " + especiales, 
					"Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Stop);
				this.txtClaveNueva.Focus();
				return;
			}
			#endregion Valida estructura clave
			...
			InsertarHistorialClave(MenuLatinium.IdUsuarioBDDInicio, this.txtClaveActual.Text.Trim());
		...
		private void InsertarHistorialClave(int idUsuario, string clave)
		{
			string cadenaConexion = @"Password=Bl45o6$9;Persist Security Info=True;User ID= 'Infoelect';Initial Catalog='INICIO';Data Source='192.168.1.165,56399'";
			SqlConnection conexion = new SqlConnection(cadenaConexion);

			try
			{
				string sqlInsert = "INSERT INTO HistorialClaveUsuarios (idUsuario, Contrasena) VALUES (@idUsuario, @Contrasena)";
				SqlCommand cmd = new SqlCommand(sqlInsert, conexion);
				cmd.Parameters.Add(new SqlParameter("@idUsuario", idUsuario));
				cmd.Parameters.Add(new SqlParameter("@Contrasena", clave));
			...
			{
				MessageBox.Show("Error al insertar en HistorialClaveUsuarios: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
			finally
			{
		...
		private bool claveVisibledOS = false;
		...
		private void label5_Click(object sender, System.EventArgs e)
		{
			...
			{
				// Mostrar clave
				this.txtClaveActual.PasswordChar = '\0'; // Sin carácter = muestra texto plano
				this.label5.Text = "Ocultar";
				claveVisible = true;
			}
		}
		...
		private void label6_Click(object sender, System.EventArgs e)
		{
			if (claveVisibledOS)
			{
				...
				this.txtRepitaClave.PasswordChar  = '*';
				this.label6.Text = "Mostrar";
				claveVisibledOS = false;
			}
			else
			{
				// Mostrar clave
				this.txtClaveNueva.PasswordChar = '\0'; // Sin carácter = muestra texto plano
				this.txtRepitaClave.PasswordChar = '\0'; // Sin carácter = muestra texto plano
				this.label6.Text = "Ocultar";
				claveVisibledOS = true;
			}
		}
	}
}
