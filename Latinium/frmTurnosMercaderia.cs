...
namespace Latinium
{
	...
	public class frmTurnosMercaderia : System.Windows.Forms.Form
	{
		...
		public frmTurnosMercaderia(int IdCompra, int Bodega, string Numero,bool BOrden)
		{
			InitializeComponent();
		...
		private void uGridTurnos_ClickCellButton(object sender, Infragistics.Win.UltraWinGrid.CellEventArgs e)
		{
			...
			bool celdaVacia = (e.Cell.Row.Cells[sColumna].Value == null) ||
				(e.Cell.Row.Cells[sColumna].Value.ToString().Trim().Length == 0);
			...
			if (celdaVacia)
			{
				...
				DateTime dtHLLegada = DateTime.Parse(sFecha + " " + sHorallegada + ":" + sColumna.Substring(3, 2));

				if (bOrden)
				{	
					DateTime inicio = dtHLLegada;
					DateTime finSugerido = inicio.AddMinutes(60);

					using (frmOrdenesDeCompra oc = new frmOrdenesDeCompra(idCompra, 1, inicio, sColumna))
					{
						oc.StartPosition = FormStartPosition.CenterParent;
						oc.ShowDialog(this); 
					}
				...
				using (frmbotones Opciones = new frmbotones())
				{
					if (Opciones.ShowDialog(this) == DialogResult.OK)
					{
						...
						{
							using (frmOrdenesDeCompra OCB = new frmOrdenesDeCompra(0, 1, dtHLLegada, sColumna))
							{
								OCB.StartPosition = FormStartPosition.CenterParent;
								OCB.ShowDialog(this);
							}
						}
						else if (Opciones.iOrigen == 2) 
						{
							using (frmGeneraOrdenesDeProduccion OP = new frmGeneraOrdenesDeProduccion(0, 2, dtHLLegada, sColumna))
							{
			...
			int idDocumento = Funcion.iEscalarSQL(cdsSeteoF,
				string.Format("Exec ConsultaIdDocumentoTurnosMercaderia '{0}', {1}", sColumna, (int)e.Cell.Row.Cells["idTurno"].Value));

			if (sNumero.Length >= 2 && sNumero.Substring(0, 2) == "OC")
			{
				using (frmOrdenesDeCompra OC = new frmOrdenesDeCompra(idDocumento))
				{
					OC.StartPosition = FormStartPosition.CenterParent;
					OC.ShowDialog(this);
				}
			}
			else if (sNumero.Length >= 2 && sNumero.Substring(0, 2) == "OP")
			{
				using (frmGeneraOrdenesDeProduccion OP = new frmGeneraOrdenesDeProduccion(idDocumento))
				{
					OP.StartPosition = FormStartPosition.CenterParent;
					OP.ShowDialog(this);
				}
