...
namespace Latinium
{
	...
	public class frmDTIngresoPorRetiro : DevExpress.XtraEditors.XtraForm
	{
		...
		private void frmDTIngresoPorRetiro_Load(object sender, System.EventArgs e)
		{
			...
			if (!Funcion.Permiso("533", cdsSeteoF))
			{				
				MessageBox.Show("No Puede Ingresar a Ingreso por Retiro Mercaderia", "Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Stop);
			...
			if (bEdicion)
			{
				...
				if (e.Cell.Column.ToString() == "Codigo")
				{
					...
					FuncionesInventario.FacturacionCargaArticulos(sender, e, this.uGridDTIngreso, 9, 4, 
						(int)this.cmbBodega.Value, 0, 0, 0, 0, 0, Convert.ToDateTime(this.dtFecha.Value), 54, this.chkFiscal.Checked, false, false, false, 0, idEntidadFinanciera, 0, 0, 0, false, false, 0, cdsSeteoF, false,0, false);
					Total();
					bActualiza = true;					
				} 
