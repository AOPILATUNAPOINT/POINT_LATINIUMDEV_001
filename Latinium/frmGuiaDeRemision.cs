...
//				this.cmbResponsable.DataSource = FuncionesProcedimientos.Lista_Choferes(Convert.ToDateTime(this.dtIniTraslado.Value), (int)this.cmbBodega.Value, cdsSeteoF);
			...
			//	this.cmbResponsable.DataSource = FuncionesProcedimientos.Lista_Choferes(Convert.ToDateTime(this.dtIniTraslado.Value), (int)this.cmbBodega.Value, cdsSeteoF);
				this.cmbResponsable.DataSource = Funcion.dvProcedimiento(cdsSeteoF, string.Format("select idChoferes as idPersonal, Nombre from DBO.choferes where bActivo = 1"));
					this.txtPlaca.Enabled = false;
			}
