...
//					s.Connect(endPt); 
		...
		static public void ModificaDescuentos(Infragistics.Win.UltraWinGrid.BeforeCellUpdateEventArgs e, bool bEmpleado)
		{		
		...
		static public bool vbIdentificacion(Infragistics.Win.UltraWinEditors.UltraTextEditor uTXT, int idTipoRuc, C1.Data.C1DataSet ds)
		{
			...
			int iTercerDigito = 0;
			if (uTXT.Text.Length > 0) iTercerDigito = int.Parse(uTXT.Text.ToString().Substring(2, 1));
...
//				}
		...
		static public bool vbValidaPlaca(Infragistics.Win.UltraWinEditors.UltraTextEditor uTXT)
		{
			...
			if (Regex.IsMatch(sLetras.Substring(1, 1), @"[EMX]"))
			{
				MessageBox.Show("La placa registrada no es valida", "Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Stop);
				uTXT.Focus();
				return false;
		...
		{
			if (Funcion.iEscalarSQL(ds, string.Format("Select COUNT(*) From Compra Where idTipoFactura = 1 And idFormaPago In (3, 9, 11) And Borrar = 0 And Estado In (10, 11) And Ruc = '{0}'", 
