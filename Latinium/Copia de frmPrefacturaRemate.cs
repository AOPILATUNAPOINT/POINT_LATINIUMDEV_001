...
namespace Latinium
{
	...
	public class frmPrefacturaRemate : System.Windows.Forms.Form
	{
		...
		private void ValidaNumeroCuotas()
		{
			...
			this.txtNumCuotas.MaxValue = 12;//iMaximoNumeroCuotas;
			this.txtNumCuotas.Value = 12;//iMaximoNumeroCuotas;
		}
				...
				try
				{
					if (bActualiza) 
					{
						...
						if (e.Cell.Column.ToString() == "Codigo")
						{
							bActualiza = false;
							if (!bModificaColor)
								FuncionesInventario.FacturacionCargaArticulos(sender, e, this.ultraGrid2, 9, 4, (int)this.cmbBodega.Value, (int)this.cmbBodega1.Value, 0, 0, 0, 0, Convert.ToDateTime(this.dtFecha.Value), 43, true, true, true, true, (int)this.txtNumCuotas.Value, (int)this.txtidEntidadFinanciera.Value, (int)this.txtNumIdCliente.Value, 0, RegistroIdSerial, false, false, 0, cdsSeteoF);
		...
		private void Consulta(int idCompra)
		{
			...
			{				
				...
				if (!bAnulado)
				{
					...
					if (miAcceso.BAprobarPrefactura || miAcceso.BRechazarPrefactura) this.btnAprobacion.Enabled = true;
