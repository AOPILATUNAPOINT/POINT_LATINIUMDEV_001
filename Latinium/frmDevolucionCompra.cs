...
namespace Latinium
{
	...
	public class frmDevolucionCompra : DevExpress.XtraEditors.XtraForm
	{
		...
		public frmDevolucionCompra(string SRuc, string SNombre,decimal DTotal, bool BRebates,int IdLiquidacionRebates)
		{
			//
			// Required for Windows Form Designer support
			//
		...
		private void InitializeComponent()
		{
			...
			this.btnDistribucion.Location = new System.Drawing.Point(904, 8);
			...
			foreach(Infragistics.Win.UltraWinGrid.UltraGridRow dr in ultraGrid1.Rows.All)
			{
				...
				if (dr.Cells["Cantidad"].Value != DBNull.Value && dr.Cells["Precio"].Value != DBNull.Value && dr.Cells["DescuentoPorc"].Value != DBNull.Value)
				{
					...
					if (Convert.ToDecimal(dr.Cells["Impuesto"].Value) > 0)
					{
						...
						dIvaLineal = ((Convert.ToDecimal(dr.Cells["Subtotal"].Value) - Convert.ToDecimal( dr.Cells["DescuentoArt"].Value)) * Convert.ToDecimal(dr.Cells["Impuesto"].Value))/100.00m;
						dIIVATotal = dIIVATotal + dIvaLineal;
					}
					else
					{
				...
				this.txtIvaTotal.Value = dIIVATotal;// Math.Round((Convert.ToDecimal(this.txtIva.Value) - Convert.ToDecimal(this.txtDescIvaTotal.Value)) * dPorcIva /100, 2);
		...
		public void  Distribucion (int sTipo)
		{
			...
			#region  distribucion 
//			if (this.txtAutFactura.Text.Length >0 && this.txtRuc.Text.Length > 0)
//			{
...
//									string sArticulo = "";
									...
									string sSQLInf = string.Format("Exec GeneraNotasDeCreditoDistribucion {0},{1},'{2}',{3}", iValor,iCuenta,dtFechaFac.ToString("yyyyMMdd"),sTipo);
									SqlDataReader drInf = Funcion.rEscalarSQL(cdsSeteoF, sSQLInf, true);
									while (drInf.Read())
									{	
										bActualiza=false;
										this.ultraGrid1.Rows.Band.AddNew();
//										sCodigo = drInf.GetString(5);
//										sArticulo = drInf.GetString(6);
										...
										this.ultraGrid1.ActiveRow.Cells["idUnidad"].Value = drInf.GetInt32(4);
										bActualiza=true;
									}
									drInf.Close();	
									cmdTotal();
									//this.ultraGrid1.ActiveRow.Cells["Impuesto"].Value = iImpuesto;
									
//									this.ultraGrid1.ActiveRow.Cells["Codigo"].Value = sCodigo;
//									this.ultraGrid1.ActiveRow.Cells["Articulo"].Value = sArticulo;
		...
		{			
			...
			this.cmbBodega.DataSource = Funcion.dvProcedimiento(cdsSeteoF, "Select Bodega, Codigo, Nombre From Bodega Where Bodega In (1, 20,95, 129)");
			if(MenuLatinium.stDirFacturacion == "PROSERVICIOS") this.cmbBodega.DataSource = Funcion.dvProcedimiento(cdsSeteoF, "Select Bodega, Codigo, Nombre From Bodega Where Bodega In (3,4)");
			if(MenuLatinium.stDirFacturacion == "GRETASANTORINI") this.cmbBodega.DataSource = Funcion.dvProcedimiento(cdsSeteoF, "Select Bodega, Codigo, Nombre From Bodega Where Bodega In (1,2,3)");
			if(MenuLatinium.stDirFacturacion == "DENTALIS") this.cmbBodega.DataSource = Funcion.dvProcedimiento(cdsSeteoF, "Select Bodega, Codigo, Nombre From Bodega Where Bodega In (1,2,3,4,5,6,7,8)");
			...
			if (bRebates)
			{
				this.BeginInvoke(new System.Windows.Forms.MethodInvoker(this.EjecutaProcesoRebates));
			}
		}
...
//						this.txtIdTipoRuc.Value = miBusqueda.idTipoRuc;
			...
			if (this.ultraGrid1.Rows.Count > 0)
			{
				...
				{	
					foreach(Infragistics.Win.UltraWinGrid.UltraGridRow dr in this.ultraGrid1.Rows.All)
					{
						...
						decimal dImpuesto = Funcion.decEscalarSQL(cdsSeteo, sSQL);
		...
		private void ultraGrid1_BeforeRowInsert(object sender, Infragistics.Win.UltraWinGrid.BeforeRowInsertEventArgs e)
		{
			...
			string stMensaje = Funcion.sEscalarSQL(cdsSeteo, stExec);
...
//			}
			else
			{
				...
				{
					if (Facturacion.AnulacionCompraVenta ((int)this.txtNumIdCompra.Value, 6, cdsSeteoF))
					{
...
//				}	
//				else 
				if (Funcion.iEscalarSQL(cdsSeteoF, string.Format("Select COUNT(*) From HistorialAutorizacionDePago Where idAsiento = {0} And Estado = 1", (int)this.txtNumIdCompra.Value)) > 0)
				{
		...
		private void btnNuevo_Click(object sender, System.EventArgs e)
		{
			...
			this.btnConsultar.Enabled = false;
			this.btnDistribucion.Enabled=true;
			this.btnCancelar.Enabled = true;
			this.btnGuardar.Enabled = true;
		...
		private void btnGuardar_Click(object sender, System.EventArgs e)
		{
			...
			using (SqlConnection oConexion = new SqlConnection("Password=Bl45o6$9;Persist Security Info=True;User ID= " + MenuLatinium.stUsuario + ";Initial Catalog=" + MenuLatinium.stDirFacturacion + ";Data Source=" + MenuLatinium.stDirServidor + "; max pool size=1024"))
			{
				...
				try
				{
					...
					#region Inventario
					oCmdActualiza.CommandText = string.Format("Exec GuardaMovimientosInventario {0}, 6", (int)this.txtNumIdCompra.Value);
					oCmdActualiza.ExecuteNonQuery();
					#endregion Inventario
...
//
			...
			{
				MessageBox.Show("Ingrese N.Identificación.", "Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Stop);
				this.txtRuc.Focus();
				return;				
			}
//
...
//			}
		...
		private void EjecutaProcesoRebates()
		{
			if (!bRebates) return;
			...
			if (this.txtRuc    != null) this.txtRuc.Text  = (sRuc == null) ? "" : sRuc.Trim();
			if (this.txtNombre != null) this.txtNombre.Text = (sNombre == null) ? "" : sNombre.Trim();
			if (this.txtTotal  != null) this.txtTotal.Value = dTotal;

			using (frmBuscaFacturasCruzaNC miBusqueda = new frmBuscaFacturasCruzaNC(4))
			{
				string pref = (sRuc != null && sRuc.Trim().Length > 0) ? sRuc.Trim()
					: (sNombre != null ? sNombre.Trim() : "");
				miBusqueda.Prefiltro = pref;
			...
			System.Data.DataTable dt = this.ultraGrid1.Tag as System.Data.DataTable;
			if (dt == null)
			{
				dt = new System.Data.DataTable("DetalleNC");
				dt.Columns.Add("idDetCompra", typeof(int));
			...
			int idArticuloReb = 8684;
			DateTime fechaInfo = (int)this.txtNumidOrigen.Value > 0 ? Funcion.dtEscalarSQL(cdsSeteoF, string.Format("Select Fecha From Compra Where idCompra = {0}", (int)this.txtNumidOrigen.Value))	: Convert.ToDateTime(this.dtFecha.Value);

			decimal ivaVigente = Funcion.decEscalarSQL( cdsSeteoF, string.Format("Select dbo.RetornaImpuestoIVAVigente('{0}', 1)", fechaInfo.ToString("yyyyMMdd")) );
			
			string sInfo = string.Format( "Exec CargaInformArticuloCompra {0}, {1}, 6, '{2}', {3}", idArticuloReb, (int)this.txtIdGrupoCliente.Value, 	fechaInfo.ToString("yyyyMMdd"), (bool)this.chkFiscal.Checked 	);
			...
			System.Data.SqlClient.SqlDataReader rdr = Funcion.rEscalarSQL(cdsSeteoF, sInfo, true);
			if (rdr.Read())
			{
				codigoArt = rdr.GetString(0); 
				nombreArt = rdr.GetString(1);    
				idUnidad  = rdr.GetInt32(2);   				
			}
			rdr.Close();
			if (codigoArt == null) codigoArt = "";
			if (nombreArt == null) nombreArt = "";
			codigoArt = codigoArt.Trim();
			nombreArt = nombreArt.Trim();
 
			System.Data.DataRow dr = dt.NewRow();
			dr["idDetCompra"] = 0;
			dr["idCompra"] = 0;
			dr["idArticulo"] = idArticuloReb;
			dr["Codigo"] = codigoArt;    
			dr["Articulo"] = nombreArt;  
			dr["Cantidad"] = 1;
			dr["Precio"] = dTotal;   
			dr["Impuesto"] = ivaVigente;       
			dr["DescuentoArt"]  = 0m;
			dr["DescuentoPorc"] = 0m;
			dr["SubTotal"] = dTotal;    
			dr["idProyecto"] = (this.txtNumIdProyecto != null && this.txtNumIdProyecto.Value != null && this.txtNumIdProyecto.Value != System.DBNull.Value)	? Convert.ToInt32(this.txtNumIdProyecto.Value) : 0;
			dr["Tipo"] = 0;
			dr["Signo"] = -1;  
			dr["Bodega"] = (this.cmbBodega != null && this.cmbBodega.Value != null) ? Convert.ToInt32(this.cmbBodega.Value) : 1;
			dr["idOrigen"] = (this.txtNumidOrigen != null && this.txtNumidOrigen.Value != null && this.txtNumidOrigen.Value != System.DBNull.Value) ? Convert.ToInt32(this.txtNumidOrigen.Value) : 0;
			dr["idUnidad"] = idUnidad; 
