...
namespace Latinium
{
	...
	public class frmReporteAuditoriaLocal1 : System.Windows.Forms.Form
	{
		...
		public frmReporteAuditoriaLocal1(int iBodega, DateTime Fecha, int IEstadoConteo)
		{
			//
...
//			ModificaGrilla(false);	
		...
		{			
			this.cmbBodega.DataSource = FuncionesProcedimientos.dtGeneral("Select Bodega, Codigo, Nombre From Bodega Where Activo = 1 And Factura = 1 And Bodega = '"+ Bodegas+"'  Order By Nombre");
		...
		{		
			if (this.cmbBodega.ActiveRow != null)
			{
				...
				this.cmbVendedor.DataSource = Funcion.dvProcedimiento(cdsSeteoF, "SELECT  n.idPersonal, n.ApellidoPaterno+' '+n.ApellidoMaterno +' '+ n.PrimerNombre +' ' + n.SegundoNombre AS Nombre From Com_AsignacionDeVendedores c Inner Join Bodega b On b.Bodega = c.Bodega Inner Join Nomina n on n.idPersonal = C.idPersonal Where C.idCargo = 1 and b.Bodega = '"+ ibodega+"' and CONVERT(Date,'"+ Convert.ToDateTime(this.dtFecha.Value).ToString("yyyyMMdd") +"') Between CONVERT(Date, Desde) And CONVERT(Date, Hasta)");
	   	
				this.cmbVendedor.Value = Funcion.iEscalarSQL(cdsSeteoF, 
					string.Format("SELECT  n.idPersonal  From Com_AsignacionDeVendedores c Inner Join Bodega b On b.Bodega = c.Bodega Inner Join Nomina n on n.idPersonal = C.idPersonal Where C.idCargo = 1 and b.Bodega = {0} and CONVERT(Date,'{1}') Between CONVERT(Date, Desde) And CONVERT(Date, Hasta)",
