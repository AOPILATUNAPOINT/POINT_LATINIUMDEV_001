...
namespace Latinium
{
	...
	public class frmCuentasPorPagar : DevExpress.XtraEditors.XtraForm
	{
		...
		private void DetalleCuentas()
		{
			...
			{				
				string sSQL = string.Format("Exec DetalleCuentasPorPagar {0}, {1}", (int)this.uGridDocumentos.ActiveRow.Cells["idCompra"].Value, (int)this.uGridDocumentos.ActiveRow.Cells["idTipoFactura"].Value);
			...
			if (e.Cell.Column.ToString() == "btnDocumento")
			{
				if (e.Cell.Row.Cells["idHistorialAutorizacionDePago"].Value != System.DBNull.Value)
				{
					...
					if ((int)e.Cell.Row.Cells["idFormaPago"].Value == 10)
					{
						...
						if (iRes)
						{
							frmRetenciones Retencion = new frmRetenciones((int)this.uGridDocumentos.ActiveRow.Cells["idCompra"].Value, this.uGridDocumentos.ActiveRow.Cells["Numero"].Value.ToString(), Convert.ToDecimal(this.uGridDocumentos.ActiveRow.Cells["Total"].Value), true);
