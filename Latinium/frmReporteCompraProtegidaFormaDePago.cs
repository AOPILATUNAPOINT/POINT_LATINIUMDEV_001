...
{
	/// <summary>
	/// Summary description for frmReporteCompraProtegidaFormaDePago.
	/// </summary>
	public class frmReporteCompraProtegidaFormaDePago : DevExpress.XtraEditors.XtraForm
	{
	...
	//	int IdBodega= 0;
		...
		public frmReporteCompraProtegidaFormaDePago()
		{
			//
		...
		public frmReporteCompraProtegidaFormaDePago(int idCompra, decimal dValor, int iOrigen, DateTime DTFecha, bool bAnticipo, decimal dValorAnticipo, bool BModificaCaja, int idCliente, int idBodega, int idFormaPago)
		{
			InitializeComponent();
		...
		public frmReporteCompraProtegidaFormaDePago(int idFormaPago, decimal dValor, int iOrigen, bool bPagoCredito, DateTime DTFecha, int idBodega, bool bFiniquitos)
		{
			InitializeComponent();
		...
		public frmReporteCompraProtegidaFormaDePago(int idCompra, int idFormaPago, decimal dValor, int iOrigen, DateTime DTFecha, int idBodega, bool bFiniquitos)
		{
			InitializeComponent();
		...
		public frmReporteCompraProtegidaFormaDePago(int idCompra, int iFormaPag, decimal  dValor, decimal  dEfectivo, decimal  dSaldo, bool bAnticipo, decimal  dAnticipo, bool bNuevo, string sNumNC, decimal dRetenido, int iOrigen, int iEstado, bool bPagoCredito, int idCliente)
		{
			//
			// Required for Windows Form Designer support
			//
		...
		private void InitializeComponent()
		{
			...
			// 
			// frmReporteCompraProtegidaFormaDePago
			// 
			...
			this.Name = "frmReporteCompraProtegidaFormaDePago";
			this.ShowInTaskbar = false;
			this.StartPosition = System.Windows.Forms.FormStartPosition.CenterParent;
			this.Text = "Detalle de Cobro";
			this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.frmReporteCompraProtegidaFormaDePago_KeyDown);
			this.Load += new System.EventHandler(this.frmReporteCompraProtegidaFormaDePago_Load);
		...
		public void frmReporteCompraProtegidaFormaDePago_Load(object sender, System.EventArgs e)
		{			
			...
			this.cmbFormaPago.DataSource = Funcion.dvProcedimiento(cdsSeteoF, string.Format("Exec FormasDePagoIngresoDeCaja {0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}, {8}", 
				IdFormaDePago, IdOrigen, BAnticipoFact, BRetencionFact, BNotaDeCreditoFact, BCredito, bModificaCaja, IdBodega, BFiniquitos));
...
//							IdFormaDePago = 1;
							...
							string sSQL = string.Format("Exec ConsultaDetalleFormaPagoNC {0}, {1}", IdNotaDeCredito, IdOrigen);
			...
			ControlSeguroLiquidado();
			SumaTotalSeguroLiquidado();
			#endregion SeguroLiquidado
		}

		#region SeguroLiquidado
		public void ControlSeguroLiquidado()
		{
			//Seguro a liquidar consulta
			string squery = String.Format("Exec CompraModel '{0}',{1}", "SeguroALiquidar", IdCompraFact);
			SeguroLiquidado = Funcion.bEjecutaSQL(cdsSeteoF,squery);
			//Consultar DepositoDeducible
			string squery1 = String.Format("Exec CompraModel '{0}',{1}", "DepositoDeducible", IdCompraFact);
			DepositoDeducible = Funcion.bEjecutaSQL(cdsSeteoF, squery1);

			if(SeguroLiquidado)
			{
				// 25 es deposito aseguradora
				// 26 deposito deducible que representa al cliente.
				squery = string.Format("EXEC ReporteCompraProtegidaDetalleFormaPagoValor {0},{1},{2}", IdCompraFact, IdOrigen, frmReporteCompraProtegida.iCompraFormaPagoDepositoAseguradora);
				ValorDocumentoEnviado = 0;
				try
				{
					ValorDocumentoEnviado = Funcion.decEscalarSQL(cdsSeteoF, squery);
				}
				catch(Exception ex)
				{
				...
				{
					squery = string.Format("EXEC ReporteCompraProtegidaDetalleFormaPagoValor {0},{1},{2}", IdCompraFact, IdOrigen, frmReporteCompraProtegida.iCompraFormaPagoDepositoTransferencia);
					ValorProductoRemplazo = 0;

					try
					{
						ValorProductoRemplazo = Funcion.decEscalarSQL(cdsSeteoF, squery);
					}
					catch(Exception ex)
					{
						string sms = ex.Message;
		...
		public void SumaTotalSeguroLiquidado()
		{
			if(SeguroLiquidado)
			{
				this.spnSubTotal.Value = Math.Round(ValorProductoRemplazo, 2) + Math.Round(ValorDocumentoEnviado, 2);
				this.spnTotal.Value = Math.Round(DValorFact, 2); //+ Math.Round(ValorDocumentoEnviado, 2);
				this.spnSaldo.Value = Convert.ToDecimal(this.spnTotal.Value) - Convert.ToDecimal(this.spnSubTotal.Value);
			}
		}
		#endregion SeguroLiquidado
		...
		private void ultraGrid1_BeforeRowsDeleted(object sender, Infragistics.Win.UltraWinGrid.BeforeRowsDeletedEventArgs e)
		{
			if (!bEliminaAlValidar)
			{
				...
				{
					if ((int)e.Rows[i].Cells["idFormaPago"].Value == 6 || (int)e.Rows[i].Cells["idFormaPago"].Value == 9 || (int)e.Rows[i].Cells["idFormaPago"].Value == 10) e.Cancel = true;
...
//				}
		...
		private void frmReporteCompraProtegidaFormaDePago_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{
...
//					}
		...
		private void ultraGrid1_BeforeCellUpdate(object sender, Infragistics.Win.UltraWinGrid.BeforeCellUpdateEventArgs e)
		{
			...
			if (e.Cell.Column.ToString() == "idFormaPago")
			{
				if (e.Cell.Row.Cells["idFormaPago"].Value != System.DBNull.Value)
				{
					if ((int)e.Cell.Row.Cells["idFormaPago"].Value == 6 || 
						(int)e.Cell.Row.Cells["idFormaPago"].Value == 9 || (int)e.Cell.Row.Cells["idFormaPago"].Value == 10)//(int)e.Cell.Row.Cells["idFormaPago"].Value == 12 ||  || (int)e.Cell.Row.Cells["idFormaPago"].Value == 11)
					{
						e.Cancel = true;
					}					
			...
			if (e.Cell.Column.ToString() == "Valor")
			{
				if (e.Cell.Row.Cells["idFormaPago"].Value != System.DBNull.Value)
				{
					//comentadp para feria ayugsii
					if (/*(int)e.Cell.Row.Cells["idFormaPago"].Value == 6 
						|| */(int)e.Cell.Row.Cells["idFormaPago"].Value == 9 || (int)e.Cell.Row.Cells["idFormaPago"].Value == 10)// || (int)e.Cell.Row.Cells["idFormaPago"].Value == 14)
					{
						e.Cancel = true;
					}
		...
		private void btGrabar_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				foreach(Infragistics.Win.UltraWinGrid.UltraGridRow dr in ultraGrid1.Rows.All)
				{
					...
					if (iFP == 25) FormaPago = "l depósito de la aseguradora";
					if (iFP == 26) FormaPago = "l depósito deducible";
