...
namespace Latinium
{
	...
	public class frmGarantiaExtendida : System.Windows.Forms.Form
	{
		...
		public frmGarantiaExtendida(Infragistics.Win.UltraWinGrid.UltraGrid UGridVenta, int IPlazo, DateTime DTFecha, int IdEntidadFinanciera, int IdTipoPrecio, bool BCP)
		{
			//
			// Necesario para admitir el Diseñador de Windows Forms
			//
		...
		{			
			...
			{	
				...
				{	
					if ((int)dr.Cells["idArticulo"].Value != 0)
					{
						...
						string sSQL = string.Format("Exec InformacionArticulosGEx {0}, {1}, {2}, {3}", (int)dr.Cells["idArticulo"].Value, idTipoPrecio, iPlazo, bCP);
			...
			#region Compra protegida obligatoria para celulares bloqueada con ticket CREQ-00002148			
//			if (iPlazo == 12 && idTipoPrecio == 4)
//			{
//				foreach(Infragistics.Win.UltraWinGrid.UltraGridRow dr2 in this.ultraGrid1.Rows.All)
//				{
//					if ((int)dr2.Cells["TipoProducto"].Value == 2)
//					{
//						if (Funcion.iEscalarSQL(cdsSeteoF, string.Format("Select idSubGrupo From Articulo Where idArticulo = {0}", (int)dr2.Cells["idArticulo"].Value)) == 23)
//						{
//							dr2.Cells["Sel"].Value = true;
...
//				}
//			}
			#endregion Compra protegida obligatoria para celulares bloqueada con ticket CREQ-00002148
				...
				if ((bool)dr.Cells["Sel"].Value)
				{
					...
					foreach(Infragistics.Win.UltraWinGrid.UltraGridRow dr1 in this.ultraGrid1.Rows.All)			
						if ((bool)dr1.Cells["Sel"].Value)				
							if ((int)dr1.Cells["idArticulo"].Value == idArticulo) iCont1++;
			
					if (iCont1 > 1)
					{
						MessageBox.Show("Solo puede seleccionar un intangible por producto", "Point Technology", MessageBoxButtons.OK, MessageBoxIcon.Stop);						
						return;
					}
				}
			}
