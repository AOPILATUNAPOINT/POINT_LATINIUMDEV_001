...
namespace Latinium
{
	...
	private Acceso miAcceso;
		...
		{			
			miAcceso = new Acceso(cdsSeteoF, "0409");

			if (miAcceso.CambiarNumeracion) this.txtNombre.Enabled = true;
			//if (MenuLatinium.stDirFacturacion == "POINT") 
			
			if (miAcceso.CambiarFecha)
			{
				this.dtFecha.Enabled = true;
				this.dtFechaCheque.Enabled = true;
			}
			...
			if (miAcceso.BVistas)
			{
				if (miAcceso.Editar)
				{
					...
					this.dtFecha.CalendarInfo.MinDate = Convert.ToDateTime(dtFechaPago);
					this.dtFechaCheque.CalendarInfo.MinDate = Convert.ToDateTime(dtFechaPago);
				}
				else
				{
					this.dtFechaCheque.CalendarInfo.MinDate = DateTime.Today;
					//this.dtFechaCheque.CalendarInfo.MaxDate = DateTime.Today;
			...
			{
				DateTime fechaLimite = new DateTime(2025, 1, 1); 
				this.dtFecha.CalendarInfo.MinDate = fechaLimite;
				this.dtFechaCheque.CalendarInfo.MinDate = fechaLimite;
			}
			...
			if (IdFormaPago == 2 && MenuLatinium.stDirFacturacion == "POINT") this.txtNumCheque.Enabled = false;
		...
		{
			if (!Validacion.vbFechaEnDocumentos(this.dtFecha, "Seleccione la Fecha Del Egreso", false, "", cdsSeteoF)) return;
			if (!Validacion.vbFechaEnDocumentos(this.dtFechaCheque, "Seleccione la Fecha De cobro", false, "", cdsSeteoF)) return;
...
//			if (!Validacion.vbComparaFechas(Convert.ToDateTime(this.dtFecha.Value), Convert.ToDateTime(this.dtFechaRetorna.Value), this.dtFecha, " La Fecha Del Egreso no puede ser Mayor a la Fecha De Cobro", false)) return;
//			
