...
//			this.cmbBodega.DataSource = Funcion.dvProcedimiento(cdsSeteoF, "Select Bodega, Nombre, Codigo From Bodega Where Activo = 1 And Factura = 1 And Caja = 1 Order By Nombre");
			this.cmbBodega.DataSource = Funcion.dvProcedimiento(cdsSeteoF, "Select Bodega, Nombre, Codigo From Bodega Where Activo = 1 And Factura = 1 And Caja = 1 AND (ISNULL (DATEPART(Year, FechaCierre),'2020') )> '" + Convert.ToDateTime(dtFechaFinal).Year.ToString()+ "' Order By Nombre");
