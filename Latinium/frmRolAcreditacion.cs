...
namespace Latinium
{
	...
	public class frmRolAcreditacion : System.Windows.Forms.Form
	{
		...
		private void archivoTxtGuayaquil()
		{
			#region Exporta
			try
			{
				...
				if (sArchivo.ShowDialog() == DialogResult.OK)
				{
					...
					{						
						...
						oCmdActualiza.Connection = oConexion;
						oCmdActualiza.CommandTimeout = 0;
						string sSQL = string.Format("Exec RolListaPersonalGuayaquil {0}, {1}, {2}", idRolNomina, idBanco, iRolComisionDecimo);
						oCmdActualiza.CommandText = sSQL;
						SqlDataAdapter oDADetalle = new SqlDataAdapter(oCmdActualiza);
						DataTable oDTDetalle = new DataTable();
						oDADetalle.Fill(oDTDetalle);	
		...
		private void RolListaPersonalGuayaquilNomina()
		{
			#region Exporta
			try
			{
				...
				if (sArchivo.ShowDialog() == DialogResult.OK)
				{
					...
					{						
						...
						oCmdActualiza.Connection = oConexion;
						oCmdActualiza.CommandTimeout = 0;
						string sSQL = string.Format("Exec RolListaPersonalGuayaquilNomina {0}, {1}, {2}", idRolNomina, idBanco, iRolComisionDecimo);
		...
		private void btnAcreditar_Click(object sender, System.EventArgs e)
		{
			...
			if(iRolComisionDecimo == 4){
				if (Funcion.bEscalarSQL(cdsSeteoF, string.Format("Exec NominaValidaRolPagado '{0}'", Convert.ToDateTime(dtHoy).ToString("yyyyMMdd")), true))
				{
			...
			if (iRolComisionDecimo == 2) sSQL = string.Format("Exec AsientoDecimoTercerSueldo {0}, {1}, {2}, '{3}'", (int)this.txtIdAsiento.Value, idRolNomina, (int)this.cmbCuenta.Value, Convert.ToDateTime(this.dtpFecha.Value).ToString("yyyyMMdd"));
			if (iRolComisionDecimo == 4) sSQL = string.Format("Exec AsientoAcreditacionBonoSemanal {0}, {1}, {2}, '{3}'", (int)this.txtIdAsiento.Value, idRolNomina, (int)this.cmbCuenta.Value, Convert.ToDateTime(this.dtpFecha.Value).ToString("yyyyMMdd"));
			//if (iRolComisionDecimo == 5) sSQL = string.Format("Exec AsientoAcreditacionAnticipoUtilidades {0}, {1}, {2}, '{3}'", (int)this.txtIdAsiento.Value, idRolNomina, (int)this.cmbCuenta.Value, Convert.ToDateTime(this.dtpFecha.Value).ToString("yyyyMMdd"));
			if (iRolComisionDecimo == 5) sSQL = string.Format("Exec AsientoRolAcreditacionAntipoUtilidades {0}, {1}, {2}, '{3}'", (int)this.txtIdAsiento.Value, idRolNomina, (int)this.cmbCuenta.Value, Convert.ToDateTime(this.dtpFecha.Value).ToString("yyyyMMdd"));
			if (iRolComisionDecimo == 6) sSQL = string.Format("Exec AsientoAcreditacionAnticipoUtilidadesNomina {0}, {1}, {2}, '{3}'", (int)this.txtIdAsiento.Value, idRolNomina, (int)this.cmbCuenta.Value, Convert.ToDateTime(this.dtpFecha.Value).ToString("yyyyMMdd"));
		...
		private void btnGenerarArchivo_Click(object sender, System.EventArgs e)
		{
			...
			if (idBanco == 5 || idBanco == 7 || idBanco == 21 || idBanco == 25)
				if (MenuLatinium.stIdDB == 8) 
					this.ArchivoTXTInternacionalCompleto();  
				else this.ArchivoTXT();

			if( idBanco == 4 && iRolComisionDecimo > 0)			
				this.archivoTxtGuayaquil();
			
			if( idBanco == 4 && iRolComisionDecimo == 0)
				this.RolListaPersonalGuayaquilNomina();

			if (MenuLatinium.stIdDB == 50) if (idBanco == 1)  this.ArchivoTXTInternacionalCompleto();
			if (MenuLatinium.stIdDB == 51) if (idBanco == 1)  this.ArchivoTXTInternacionalCompleto();
			if (MenuLatinium.stIdDB == 55) if (idBanco == 1)  this.ArchivoTXTInternacionalCompleto();
