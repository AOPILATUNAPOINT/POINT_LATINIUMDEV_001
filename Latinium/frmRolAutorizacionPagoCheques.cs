	...
	public class frmRolAutorizacionPagoCheques : System.Windows.Forms.Form
	{
		...
		private void uGridListaCheques_AfterCellUpdate(object sender, Infragistics.Win.UltraWinGrid.CellEventArgs e)
		{
			if (e.Cell.Column.ToString() == "idCuenta")
			{
				...
				else if (iRolComisionDecimo == 4)
				{
					string sSQL = string.Format("Update BonoSemanal Set idCuenta = {0} Where IdBonoSemanal = {1}", idCuenta, (int)e.Cell.Row.Cells["idDetalleRolNomina"].Value);
					Funcion.EjecutaSQL(cdsSeteoF, sSQL);
				}
			}
		}
			...
			if (DialogResult.Yes == MessageBox.Show("Esta Seguro de Autorizar los Pagos", "Point Technology", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2)) 
			{
				...
				foreach(Infragistics.Win.UltraWinGrid.UltraGridRow dr in this.uGridListaCheques.Rows.All)
				{
					if ((int)dr.Cells["idRolPrestamo"].Value == 0)
					{
						...
						if (iRolComisionDecimo == 4) idTipo = 25;
						if (iRolComisionDecimo == 5) idTipo = 27;
						if (iRolComisionDecimo == 6) idTipo = 27;

						string sNumero = Funcion.sEscalarSQL(cdsSeteoF, string.Format("Select dbo.SecuencialPrestamoEmpleados({0})",idTipo));
						...
						else if (iRolComisionDecimo == 4) sMensaje = "COMISIONES SEMANALES POR PAGAR DEL PERIODO : " + dtFecha.ToString("MMMM/yyyy");
						else if (iRolComisionDecimo == 5) sMensaje = "ANTICIPO UTILIDADES POR PAGAR DEL PERIODO : " + dtFecha.ToString("MMMM/yyyy");
						...
						string sSQLAut = string.Format("Exec CreaAutorizacionesDePago {0}, {1}, '{2}', '{3}', {4}, 2, '1 DE 1', '{5}', {6}, 1", 
						...
						Funcion.EjecutaSQL(cdsSeteoF, string.Format("Update AutorizacionDePagos Set Estado = 0, Saldo = Valor Where idTipo = {1} And idOrigen = {0}", (int)dr.Cells["idRolPrestamo"].Value, idTipo));
